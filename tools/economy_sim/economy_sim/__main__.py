...
def main() -> int:
    ...
    parser.add_argument("--emission", type=float, default=EconomyConfig.base_emission_per_season,
                        help="Emission pool at initial population, before caps")
    parser.add_argument("--vault", type=float, default=EconomyConfig.rewards_vault,
                        help="Rewards vault size; 0 switches to unbounded minting mode")
    ...
    if args.json:
        ...
        print(f"{'S':>2} {'players':>9} {'earners':>8} {'cap':>12} {'emitted':>12} {'burned':>12} "
              f"{'circ':>13} {'vault':>13} {'price_idx':>10} {'per_earner':>11}")
        for s in report.seasons:
            print(f"{s.season:>2} {s.players:>9} {s.earners:>8} {s.season_cap:>12,.0f} {s.emitted:>12,.0f} "
                  f"{s.burned:>12,.0f} {s.circulating:>13,.0f} {s.vault_remaining:>13,.0f} "
        ...
        if config.rewards_vault > 0:
            ex = report.vault_exhausted_season
            print(f"vault exhausted:      {'season ' + str(ex) if ex else 'no'}")
